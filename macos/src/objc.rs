...
pub struct Arc<T: ?Sized> { // NOTE: added the ?Sized because protocol methods require it.
    ptr: NonNull<T>,
}
...
impl<T: ?Sized> Drop for Arc<T> {  // Added ?Sized here because it was added above.
    fn drop(&mut self) {
