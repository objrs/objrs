...
// The original code used std::collections::HashMap.
// Switched to IndexMap so that the ordering of types in the output is deterministic.
// Otherwise, the order changed with every build.
    ...
    pub fn raw_ty(&self) -> syn::Type {
        match self {
            ...
            Type::FixedArray(inner, len) => {
                ...
                let array_len =
                    syn::LitInt::new(&len.to_string()[..], Span::call_site());
                    //syn::LitInt::new(*len, syn::IntSuffix::None, Span::call_site());
            ...
            Type::FunctionProto(args, retty, var) => {
                ...
                if let syn::Type::BareFn(syn::TypeBareFn { ref mut variadic, .. }) = f {
                    if *var {
						panic!("Not implemented variadic #1")
...
impl MethodDecl {
	...
	pub fn gen_call(&self, decls: &IndexMap<String, ItemDecl>, s: &str, 
		class: bool,
		is_protocol: bool) -> 
...
struct ClassDecl {
    ...
    cprops: IndexMap<String, PropertyDecl>,
    iprops: IndexMap<String, PropertyDecl>,
    cmethods: IndexMap<String, MethodDecl>,
    imethods: IndexMap<String, MethodDecl>,
}

impl ClassDecl {
	pub fn read(c: &walker::Cursor, framework_name: Option<&str>) -> ClassDecl {
        println!("{}", c.name());
		...
		let framework_name: String = match framework_name {
			None => String::from(""),
			Some(ss) => ss.into()
		};
        ...
        let mut decl = ClassDecl {
            ...
            cprops: IndexMap::new(),
            iprops: IndexMap::new(),
            cmethods: IndexMap::new(),
            imethods: IndexMap::new(),
        };
...
impl ItemDecl {
	...
	// The framework name is the stem of the nearest folder
	// whose name ends in .Framework
	// Returns a vector of framework names, starting at the innermost,
	// i.e. in bottom-up order
	...
	// The file name is the stem of the source file
    fn file_name(&self) -> String {
		return self.src().file_stem().unwrap().to_str().unwrap().to_owned();
	}
...
pub fn bind_framework(
	...
	let sdk_path = get_sdk_root(&target_os);
	...
	if is_ios(&target_os) && framework_name == "CoreAudio" {
		include_path.push("CoreAudioTypes.h");
	} else if is_ios(target_os) && framework_name == "OpenGLES" {
		include_path.push("EAGL.h");
	} else {
...
/// Objective-C protocols and classes may have the same name.
/// An example is NSObject, which is both a protocol and a class.
/// The original code adds a Proto suffix to all protocols, but this 
/// is not ergonomic; users expect the original protocol name wherever 
/// possible.
fn objrs_protocol_display_name(name: &String) -> String {
	match name.as_ref() {
		"NSObject" => String::from("NSObjectProtocol"),
		_ => name.to_string()
	}
}

/// Binds a translation unit.
/// The translation unit may be a standalone file, e.g. NSObject.h,
/// or it may be part of a framework.
/// * `framework_name`: None if the translation unit is an independent file.
pub fn bind_tu(
	...
	// refactored this function so that we can do the uniqueness check in a single place
	fn insert_decl(decls: &mut IndexMap<String, ItemDecl>, name: String, decl: ItemDecl) -> Option<ItemDecl>{
		if name.contains("NSAccessibilityElement") {
			let contains = decls.contains_key("NSAccessibilityElement");
			let has = contains;
		}

		let old = decls.insert(name.clone(), decl);
		if old.is_some() {
			panic!("??? decl {} already defined", &name);
		}
		return old;
	};
    ...
    tu.visit(|c| {
        ...
        match c.kind() {
            CursorKind::ObjCCategoryDecl => {
                let class = ClassDecl::read(&c, framework_name);
            ...
            CursorKind::ObjCInterfaceDecl => {
                ...
                let old = insert_decl(&mut decls, name.clone(), ItemDecl::Class(class));
                declnames.push(name);
            },
            CursorKind::ObjCProtocolDecl => {
				let mut name = c.name();

				// Add a "Proto" suffix if emitting for rustkit, but not for objrs
				if cfg!(feature = "objrs") {
					//name = objrs_protocol_display_name(&name);
					name.push_str("Proto");
				}
				else {
					name.push_str("Proto");
                ...
                let proto = ClassDecl::read(&c, framework_name);
                if c.location().filename().starts_with(base_path) {
                    println!("{:#?}", proto);
                    cursor_dump(&c, None);
                }
                insert_decl(&mut decls, name.clone(), ItemDecl::Proto(proto));
                declnames.push(name);
            },
            CursorKind::EnumDecl => {
                let name = c.name();
                ...
                insert_decl(&mut decls, name.clone(), ItemDecl::Enum(decl));
                declnames.push(name);
            },
            CursorKind::StructDecl | CursorKind::UnionDecl => {
                let name = c.name();
	...
	// Umbrella frameworks such as CoreServices can contain other frameworks.
	// https://developer.apple.com/library/archive/documentation/MacOSX/Conceptual/BPFrameworks/Concepts/FrameworkAnatomy.html
	// In this case there is a Frameworks subdirectory
	...
	let mods = std::fs::read_dir(&subframeworks_path)
		.map(|rd| rd.map(|e| e.unwrap().path().file_stem().unwrap().to_str().unwrap().to_owned())
		.collect::<Vec<_>>()).unwrap_or(Vec::new());

    let mut deps = HashSet::new();
    if mods.is_empty() {
		gen_file(&decls, &declnames, base_path, &mods, framework_name, framework_name.is_none(), 
				 out_path, &mut deps);
        return deps;
    }
...
fn gen_file(
    decls: &IndexMap<String, ItemDecl>,
    declnames: &[String],
    base_path: &Path,
    mods: &[String],
    framework_name: Option<&str>,
	...
	fn gen_framework_sel_attr(decls: &IndexMap<String, ItemDecl>, 
		framework_name: Option<&str>, refs: &[String]) -> 
        ...
        if frameworks.is_empty() {
			...
			None // FIXME: original code below, does not compile after 
			// syn, quote and proc-macro2 were upgraded to version 1.0+.
        ...
        match decls.get(n) {
            Some(d) => {
                if d.src().starts_with(base_path) {
					...
					// build up a fully qualified reference to the declaration
					...
					if cfg!(feature = "objrs") {
						if name.is_empty() { 
							// No framework, top level. E.g. NSObject
							// Construct a qualified name using the source file name
							let comp = Ident::new(&d.file_name()[..], Span::call_site());
							path = parse_quote!{ #comp::#path };
							Some(parse_quote!{
								use crate::#path;
							})
						} else { 
							// the framework name is set, so use a crate-relative module prefix.
							Some(parse_quote!{
								use crate::#path;
							})
						}
	...
	if cfg!(feature = "objrs") {
		// We don't need to output anything in objrs
	} else  {
		// Output all the selectors to the start of the file.
		for s in selectors {
    ...
    for k in declnames {
        match decls.get(k).unwrap() {
            ItemDecl::Enum(e) => {
				...
				if false && cfg!(feature = "objrs") {
					// The objrs syntax for enums is shown below.
					// This is unweildy, so we prefer the rustkit syntax, and
					// will convert rustkit to use the objrs syntax.
					// #[repr(transparent)]
					...
					let variants: Vec<syn::Variant> = e.variants.iter().map(|(n, v, neg)| {
						...
						let var_val =
							syn::LitInt::new(&v.to_string()[..], Span::call_site());
            ...
            ItemDecl::Class(c) => {
				...
				if cfg!(feature = "objrs") {
					...
					let name = Ident::new(&c.rustname, Span::call_site());
					if !&c.superclass.is_empty() {
						let superclass = Ident::new(&c.superclass, Span::call_site());
						// let superclass = proc_macro2::Literal::string(&c.superclass);
						ast.items.push(parse_quote!{
							#[objrs(class, super = #superclass)] 
							#[link(name = #framework_name, kind = "framework")]
							pub struct #name;
						});
					} else if let Some(_) = framework_name {
						// no superclass, but has a framework name
						ast.items.push(parse_quote!{ 
						...
						// no superclass, no framework, so omit the link attribute
						// (Is this correct?)
						ast.items.push(parse_quote!{ 
							#[objrs(class, root_class)] 
							pub struct #name;
					...
					// extern "C" {
					//     #[link_name = "OBJC_CLASS_$_NSMenuItem"]
					//     static NSMenuItemClass: Class;
					// }
					...
					// static CLASS_NSMenuItem: ClassRef = ClassRef(unsafe { &NSMenuItemClass } as *const _);
					...
					// impl ObjCClass for NSMenuItem {
					//     const START: usize = <NSObject as ObjCClass>::SIZE;
					//     const SIZE: usize = 96;
					...
					// }
					let instance_size =
						syn::LitInt::new(&c.size.to_string()[..], Span::call_site());
						//syn::LitInt::new(c.size, syn::IntSuffix::None, Span::call_site());
					...
					// impl NSCopyingProto for NSMenuItem {}
					// impl NSCodingProto for NSMenuItem {}
					// impl NSValidatedUserInterfaceItemProto for NSMenuItem {}
					// impl NSUserInterfaceItemIdentificationProto for NSMenuItem {}
					// impl NSAccessibilityElementProto for NSMenuItem {}
					// impl NSAccessibilityProto for NSMenuItem {}
					for p in &c.protocols {
						let protoname = format!("{}__", p); // PROTOCOL_SUFFIX
					...
					// collect methods into a single vector.
					...
					for (_, p) in &c.iprops {
						...
						if let Some(m) = &p.getter_method {
							if let Some(tokens) = m.gen_call(&decls, &p.getter, false, false) {
						...
						if let Some(m) = &p.setter_method {
							if let Some(tokens) = m.gen_call(&decls, p.setter.as_ref().unwrap(), false, false) {
								let mut func = syn::parse2(tokens).unwrap();
								if let syn::ImplItem::Method(ref mut method) = func {
									method.vis = parse_quote!{pub};
									if let Some(cfg) = gen_framework_sel_attr(decls, framework_name, &m.refs()) {
					...
					for (s, m) in &c.cmethods {
						if let Some(tokens) = m.gen_call(&decls, s, true, false) {
							let mut func = syn::parse2(tokens).unwrap();
							if let syn::ImplItem::Method(ref mut method) = func {
								method.vis = parse_quote!{pub};
								if let Some(cfg) = gen_framework_sel_attr(decls, framework_name, &m.refs()) {
					...
					for (s, m) in &c.imethods {
						...
						if let Some(tokens) = m.gen_call(&decls, s, false, false) {
							let mut func = syn::parse2(tokens).unwrap();
							if let syn::ImplItem::Method(ref mut method) = func {
								method.vis = parse_quote!{pub};
								if let Some(cfg) = gen_framework_sel_attr(decls, framework_name, &m.refs()) {
            ...
            ItemDecl::Proto(c) => {
				...
				if cfg!(feature = "objrs") {
					...
					for (s, m) in &c.imethods {
						if let Some(tokens) = m.gen_call(&decls, s, false, true) {
							let mut func = syn::parse2(tokens).unwrap();
					...
					//let id_ident = Ident::new(&id, Span::call_site());

					// #[objrs(protocol, name = #id_ident)]
					if let Some(_) = framework_name {
						ast.items.push(parse_quote!{
							#[objrs(protocol)]
							#[link(name = #framework_name, kind = "framework")]
							pub trait #name {
						...
						// no framework, so omit the link attribute. (Is this correct?)
						ast.items.push(parse_quote!{
							#[objrs(protocol)]
							pub trait #name {
								#(#methods)*
    ...
    let funcs: Vec<syn::ForeignItem> = decls.values().filter_map(|i| {
        ...
        if f.variadic {
			fndecl.sig.variadic = Some(syn::Variadic{
				dots: syn::Token!{...}(Span::call_site()),
				attrs: vec![]
			});
			//panic!("Not implemented: sig.variadic")
            //fndecl.sig.variadic = Some(syn::Token!{...}(Span::call_site())); //Some(syn::token::Dot3::new(Span::call_site()));
...
fn bind_system_header(sdk_root: &Path, header: &str, out_dir: &Path, top: &mut File) {
    ...
    bind_file(&sdk_root, &header_path, &out_dir);
    write!(top, "include!(concat!(env!(\"OUT_DIR\"), \"/{}.rs\"));\n", header_path.file_stem().unwrap().to_str().unwrap()).unwrap();
}

fn is_ios(target_os: &str) -> bool {
	return target_os.contains("ios");
}

fn get_sdk_root(target_os: &str) -> &Path {
	let sdk_root: &Path;
	let s: &str;

	if is_ios(&target_os) { 
		s = "/Applications/Xcode10.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator.sdk";
	} else { 
		s = "/Applications/Xcode10.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk";
	}

	return Path::new(s);
}

pub fn bind_all (target_os: &str, out_dir: &str) {
	println!("OUT_DIR={}",&out_dir) ;
	let out_dir = Path::new(&out_dir);
	let sdk_root = get_sdk_root(&target_os);
	
	// bind the system headers
	...
	if !is_ios(&target_os) { bind_system_header(&sdk_root, "hfs/hfs_unistr.h", &out_dir, &mut top); }
    bind_system_header(&sdk_root, "mach/message.h", &out_dir, &mut top);
	bind_system_header(&sdk_root, "simd/types.h", &out_dir, &mut top);

	// bind the frameworks
	let mut frameworks = vec!["AVKit", "AppKit", "Foundation"];
	if is_ios(&target_os) { frameworks = vec!["AVKit", "UIKit", "Foundation"]; }
	//else { frameworks = vec!["CoreFoundation"] } // during development, to build faster.
	else { frameworks = vec!["Foundation", "AppKit", "MetalKit"] } // during development, to build faster.

    let mut done: HashSet<String> = HashSet::new();
	let mut deps: Vec<String> = frameworks.iter().map(|s| s.to_string()).collect();
	
    while let Some(f) = deps.pop() {
        let newdeps = bind_framework(&target_os, &f, &out_dir);
        write!(top, "pub mod {};\n", f).unwrap();
        done.insert(f);
        for d in &newdeps {
